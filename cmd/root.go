...
import (
	...
	"github.com/qinqd2006/crush/crush-agent/adapter"
	"github.com/qinqd2006/crush/crush-agent/app"
	...
	"github.com/qinqd2006/crush/crush-agent/proto"
	"github.com/qinqd2006/crush/crush-agent/server"
	"github.com/qinqd2006/crush/crush-agent/session"
	"github.com/qinqd2006/crush/crush-agent/version"
	"github.com/qinqd2006/crush/crush-agent/workspace"
	"github.com/qinqd2006/crush/ui/common"
	ui "github.com/qinqd2006/crush/ui/model"
...
crush --continue
  `,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		go ws.KernelSubscribe(program)

		if _, err := program.Run(); err != nil {
			event.Error(err)
			slog.Error("TUI run error", "error", err)
			return errors.New("Crush crashed. If metrics are enabled, we were notified about it. If you'd like to report it, please copy the stacktrace above and open an issue at https://github.com/qinqd2006/crush/issues/new?template=bug.yml") //nolint:staticcheck
		}
		return nil
	},
}
...
// AppWorkspace, with a kernel.Adapter for decoupled access.
func setupLocalWorkspace(cmd *cobra.Command) (workspace.Workspace, func(), error) {
	...
	// Create the kernel adapter for decoupled access
	kernelAdapter := adapter.NewAdapter(
		store,
		appInstance.Sessions,
		appInstance.Messages,
		appInstance.Permissions,
		appInstance.History,
		appInstance.FileTracker,
		appInstance.AgentCoordinator,
		appInstance.AgentNotifications(),
		appInstance.LSPManager,
	)

	// Create AppWorkspace and wrap with KernelWorkspace
	appWs := workspace.NewAppWorkspace(appInstance, store)
	ws := workspace.NewKernelWorkspace(appWs, kernelAdapter)
