...
import (
	...
	"qinqd2006github.com/qinqd2006/crush/crush-agent/pkg/agent/hyper"
	"qinqd2006github.com/qinqd2006/crush/crush-agent/pkg/agent/notify"
	"qinqd2006github.com/qinqd2006/crush/crush-agent/pkg/agent/prompt"
	...
	"qinqd2006github.com/qinqd2006/crush/crush-agent/pkg/log"
	"qinqd2006github.com/qinqd2006/crush/crush-agent/pkg/message"
	"qinqd2006github.com/qinqd2006/crush/crush-agent/pkg/oauth/copilot"
...
type Coordinator interface {
	// SetMainAgent sets the active agent by name.
	SetMainAgent(name string) error
	// Agents returns all registered agents.
	Agents() map[string]SessionAgent
	// SessionAgent returns the current session agent.
...
func NewCoordinator(
	...
	if c.currentAgent == nil && len(c.agents) > 0 {
		// Fall back to first available agent if no coder agent.
...
func (c *coordinator) SetMainAgent(name string) error {
...
func (c *coordinator) Agents() map[string]SessionAgent {
	return c.agents
}
...
// buildAgentModels builds the large and small models for an agent.
// The large model is determined by agentCfg.Model, or overridden by
// SelectedModelTypeLarge when isSubAgent is false. The small model is
// always SelectedModelTypeSmall (used for sub-agents and summaries).
func (c *coordinator) buildAgentModels(ctx context.Context, agentCfg config.Agent, isSubAgent bool) (Model, Model, error) {
	modelKey := config.SelectedModelTypeLarge
	if isSubAgent {
		modelKey = config.SelectedModelTypeSmall
	} else if agentCfg.Model != "" {
		modelKey = agentCfg.Model
	}

	largeModelCfg, ok := c.cfg.Config().Models[modelKey]
	if !ok {
...
func (c *coordinator) UpdateModels(ctx context.Context) error {
	// Refresh models and tools for all agents.
