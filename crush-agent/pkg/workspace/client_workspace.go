...
import (
	...
	"github.com/qinqd2006/crush/infra/pkg/oauth"
	...
	remotekernel "qinqd2006github.com/qinqd2006/crush/crush-agent/pkg/adapter/remote"
...
func NewClientWorkspace(c *client.Client, ws proto.Workspace) *ClientWorkspace {
	...
	cw := &ClientWorkspace{
		client:       c,
		remoteKernel: remotekernel.NewRemoteKernel(c, ws.ID),
		ws:           ws,
	}
	return cw
}
...
func (w *ClientWorkspace) UpdatePreferredModel(scope config.Scope, modelType kernel.SelectedModelType, model kernel.SelectedModel) error {
	err := w.client.UpdatePreferredModel(context.Background(), w.workspaceID(), scope, config.SelectedModelType(modelType), config.SelectedModel{
		Model:            model.Model,
...
// Subscribe is a no-op for ClientWorkspace since kernel events are handled
// by KernelSubscribe instead.
func (w *ClientWorkspace) Subscribe(program *tea.Program) {
	// No-op: kernel events are handled by KernelSubscribe
}

// KernelSubscribe sets up kernel event subscriptions by starting the event
// translation loop and forwarding kernel events to the tea program.
func (w *ClientWorkspace) KernelSubscribe(program *tea.Program) {
	if w.remoteKernel == nil {
		return
	}

	// Start the event translation loop (RemoteKernel specific)
	if starter, ok := w.remoteKernel.(interface{ StartEventTranslation(context.Context) error }); ok {
		ctx := context.Background()
		_ = starter.StartEventTranslation(ctx)
	}

	// Forward kernel events to the tea program
	go w.forwardKernelEventsToProgram(program)
}

// forwardKernelEventsToProgram reads from kernel subscription channels and sends
// kernel events to the tea program as workspace.KernelEventMsg.
func (w *ClientWorkspace) forwardKernelEventsToProgram(program *tea.Program) {
	...
	go func() {
		defer wg.Done()
		for event := range w.remoteKernel.SubscribeSessions(ctx) {
	...
	go func() {
		defer wg.Done()
		for event := range w.remoteKernel.SubscribeMessages(ctx) {
	...
	go func() {
		defer wg.Done()
		for event := range w.remoteKernel.SubscribePermissions(ctx) {
			program.Send(KernelEventMsg[kernel.PermissionRequest]{Event: event})
		}
	}()

	// Permission notification events
	wg.Add(1)
	go func() {
		defer wg.Done()
		for event := range w.remoteKernel.SubscribePermissionNotifications(ctx) {
	...
	go func() {
		defer wg.Done()
		for event := range w.remoteKernel.SubscribeNotifications(ctx) {
	...
	go func() {
		defer wg.Done()
		for event := range w.remoteKernel.SubscribeMCP(ctx) {
	...
	go func() {
		defer wg.Done()
		for event := range w.remoteKernel.SubscribeLSP(ctx) {
	...
	go func() {
		defer wg.Done()
		for event := range w.remoteKernel.SubscribeHistory(ctx) {
...
// Kernel returns the RemoteKernel instance for ClientWorkspace.
func (w *ClientWorkspace) Kernel() kernel.Kernel {
	return w.remoteKernel
}
