...
func (w *AppWorkspace) UpdatePreferredModel(scope config.Scope, modelType kernel.SelectedModelType, model kernel.SelectedModel) error {
	return w.store.UpdatePreferredModel(scope, config.SelectedModelType(modelType), config.SelectedModel{
		Model:            model.Model,
...
// KernelSubscribe is a no-op for AppWorkspace since it doesn't use kernel.
// Use KernelWorkspace instead for kernel-based subscriptions.
func (w *AppWorkspace) KernelSubscribe(program *tea.Program) {
	// No-op: kernel subscriptions are handled by KernelWorkspace
}
...
// Kernel returns nil for AppWorkspace since it does not use a kernel-based implementation.
func (w *AppWorkspace) Kernel() kernel.Kernel {
	return nil
}

// ResetCache clears compiled regex caches used by search tools.
func (w *AppWorkspace) ResetCache() {
	tools.ResetCache()
}
