...
import (
	...
	"github.com/mosaic2025002/crush/engine/agent/notify"
	agenttools "github.com/mosaic2025002/crush/engine/agent/tools"
	...
	"github.com/mosaic2025002/crush/ui/completions"
	"github.com/mosaic2025002/crush/ui/dialog"
	fimage "github.com/mosaic2025002/crush/ui/image"
	"github.com/mosaic2025002/crush/ui/logo"
	"github.com/mosaic2025002/crush/ui/notification"
...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case workspace.KernelEventMsg[kernel.Session]:
		if cmd := m.handleKernelSessionEvent(msg.Event); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case workspace.KernelEventMsg[kernel.Message]:
		if cmd := m.handleKernelMessageEvent(msg.Event); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case workspace.KernelEventMsg[kernel.Notification]:
		if cmd := m.handleKernelNotificationEvent(msg.Event); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case workspace.KernelEventMsg[kernel.PermissionRequest]:
		if cmd := m.handleKernelPermissionRequestEvent(msg.Event); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case workspace.KernelEventMsg[kernel.PermissionNotification]:
		if cmd := m.handleKernelPermissionNotificationEvent(msg.Event); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case workspace.KernelEventMsg[kernel.MCPEvent]:
		if cmd := m.handleKernelMCPEvent(msg.Event); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case workspace.KernelEventMsg[kernel.LSPEvent]:
		m.handleKernelLSPEvent(msg.Event)
	case workspace.KernelEventMsg[kernel.HistoryFile]:
...
func (m *UI) handleKernelSessionEvent(event kernel.Event[kernel.Session]) tea.Cmd {
	// Convert kernel session to internal session for existing handlers
	sess := kernelSessionToSession(event.Payload)
...
func (m *UI) handleKernelMessageEvent(event kernel.Event[kernel.Message]) tea.Cmd {
	// Convert kernel message to internal message for existing handlers
	msg := kernelMessageToMessage(event.Payload)
	...
	if msg.SessionID != m.session.ID {
		...
		return m.handleChildSessionMessageFromKernel(msg)
	}

	switch event.Type {
	case kernel.EventCreated:
		return m.appendSessionMessageFromKernel(msg)
	case kernel.EventUpdated:
		return m.updateSessionMessageFromKernel(msg)
	case kernel.EventDeleted:
		m.chat.RemoveMessage(msg.ID)
	}
	return nil
...
func (m *UI) handleKernelNotificationEvent(event kernel.Event[kernel.Notification]) tea.Cmd {
	// Convert kernel notification to internal notification
	notif := notify.Notification{
		SessionID:    event.Payload.SessionID,
		SessionTitle: event.Payload.SessionTitle,
		Type:         notify.Type(event.Payload.Type),
		ProviderID:   event.Payload.ProviderID,
	}
	return m.handleAgentNotification(notif)
}

func (m *UI) handleKernelPermissionRequestEvent(event kernel.Event[kernel.PermissionRequest]) tea.Cmd {
	// Convert kernel permission request to internal permission request
...
func (m *UI) handleKernelPermissionNotificationEvent(event kernel.Event[kernel.PermissionNotification]) tea.Cmd {
	notif := permission.PermissionNotification{
		ToolCallID: event.Payload.ToolCallID,
		Granted:    event.Payload.Granted,
		Denied:     event.Payload.Denied,
	}
	m.handlePermissionNotification(notif)
	return nil
}

func (m *UI) handleKernelMCPEvent(event kernel.Event[kernel.MCPEvent]) tea.Cmd {
	// Convert kernel MCP event to internal mcp.Event
	mcpEvent := mcp.Event{
		Type:  kernelMCPEventTypeToMCP(event.Payload.Type),
		Name:  event.Payload.Name,
		State: mcp.State(event.Payload.State),
		Error: event.Payload.Error,
		Counts: mcp.Counts{
			Tools:     event.Payload.ToolCount,
			Prompts:   event.Payload.PromptsCount,
			Resources: event.Payload.ResourcesCount,
		},
	}
	...
	case mcp.EventPromptsListChanged:
		return handleMCPPromptsEvent(m.com.Workspace, mcpEvent.Name)
	case mcp.EventToolsListChanged:
		return handleMCPToolsEvent(m.com.Workspace, mcpEvent.Name)
	case mcp.EventResourcesListChanged:
		return handleMCPResourcesEvent(m.com.Workspace, mcpEvent.Name)
	}
	return nil
}
...
func (m *UI) handleKernelHistoryEvent(event kernel.Event[kernel.HistoryFile]) tea.Cmd {
	// Convert kernel history file to internal history.File
	file := history.File{
		Path: event.Payload.Path,
	}
	return m.handleFileEvent(file)
}

// handleKernelSessionEvent helper: convert kernel.Session to session.Session
func kernelSessionToSession(k kernel.Session) session.Session {
	todos := make([]session.Todo, len(k.Todos))
	for i, t := range k.Todos {
...
// kernelMessageToMessage helper: convert kernel.Message to message.Message
func kernelMessageToMessage(k kernel.Message) message.Message {
	parts := make([]message.ContentPart, 0, len(k.Parts))
	for _, p := range k.Parts {
		switch tp := p.(type) {
		case kernel.TextContent:
			parts = append(parts, message.TextContent{Text: tp.Text})
		case kernel.ReasoningContent:
		...
		case kernel.FinishContent:
			parts = append(parts, message.Finish{
				Reason:  message.FinishReason(tp.Reason),
				Time:    tp.Time,
				Message: tp.Message,
				Details: tp.Details,
			})
	...
	return message.Message{
		ID:               k.ID,
		Role:             message.MessageRole(k.Role),
		...
		CreatedAt:        k.CreatedAt,
		UpdatedAt:        k.UpdatedAt,
		IsSummaryMessage: k.IsSummaryMessage,
	}
}

// kernelMCPEventTypeToMCP converts kernel.MCPEventType to mcp.EventType
func kernelMCPEventTypeToMCP(t kernel.MCPEventType) mcp.EventType {
	switch t {
...
func (m *UI) appendSessionMessageFromKernel(msg message.Message) tea.Cmd {
	var cmds []tea.Cmd
...
func (m *UI) handleChildSessionMessageFromKernel(event message.Message) tea.Cmd {
	var cmds []tea.Cmd

	// Only process messages with tool calls or results.
	if len(event.ToolCalls()) == 0 && len(event.ToolResults()) == 0 {
		return nil
	}
