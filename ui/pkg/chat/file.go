...
func (v *ViewToolRenderContext) RenderTool(sty *styles.Styles, width int, opts *ToolRenderOpts) string {
	...
	if err := json.Unmarshal([]byte(opts.ToolCall.Input), &params); err != nil {
		return toolErrorContent(sty, &kernel.ToolResultContent{Content: "Invalid parameters"}, cappedWidth)
	}
	...
	if len(opts.Result.Data) > 0 && strings.HasPrefix(opts.Result.MIMEType, "image/") {
		body := toolOutputImageContent(sty, string(opts.Result.Data), opts.Result.MIMEType)
	...
	if meta.ResourceType == kernel.ViewResourceSkill {
...
func (e *EditToolRenderContext) RenderTool(sty *styles.Styles, width int, opts *ToolRenderOpts) string {
	...
	if err := json.Unmarshal([]byte(opts.ToolCall.Input), &params); err != nil {
		return toolErrorContent(sty, &kernel.ToolResultContent{Content: "Invalid parameters"}, width)
	}
