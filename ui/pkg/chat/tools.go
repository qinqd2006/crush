...
import (
	...
	"github.com/qinqd2006/crush/infra/pkg/diff"
	"github.com/qinqd2006/crush/infra/pkg/fsext"
	"github.com/qinqd2006/crush/infra/pkg/stringext"
...
type ToolMessageItem interface {
	...
	SetToolCall(tc kernel.ToolCallContent)
	SetResult(res *kernel.ToolResultContent)
	MessageID() string
...
func newBaseToolMessageItem(
	...
	hasCappedWidth := toolCall.Name != kernel.ToolNameEdit && toolCall.Name != kernel.ToolNameMultiEdit
...
func (t *baseToolMessageItem) ToolCall() kernel.ToolCallContent {
	return t.toolCall
}

// SetToolCall sets the tool call associated with this message item.
func (t *baseToolMessageItem) SetToolCall(tc kernel.ToolCallContent) {
	t.toolCall = tc
	t.clearCache()
}

// SetResult sets the tool result associated with this message item.
func (t *baseToolMessageItem) SetResult(res *kernel.ToolResultContent) {
	t.result = res
	t.clearCache()
}
...
func toolErrorContent(sty *styles.Styles, result *kernel.ToolResultContent, width int) string {
	if result == nil {
		return ""
	}
...
func toolOutputMultiEditDiffContent(sty *styles.Styles, file string, meta kernel.MultiEditResponseMetadata, totalEdits, width int, expanded bool) string {
	bodyWidth := width - toolBodyLeftPaddingTotal
...
func (t *baseToolMessageItem) formatParametersForCopy() string {
	...
	case kernel.ToolNameEdit:
		var params kernel.EditParams
		if json.Unmarshal([]byte(t.toolCall.Input), &params) == nil {
			return fmt.Sprintf("**File:** %s", fsext.PrettyPath(params.FilePath))
		}
	case kernel.ToolNameMultiEdit:
		var params kernel.MultiEditParams
		if json.Unmarshal([]byte(t.toolCall.Input), &params) == nil {
			var parts []string
			parts = append(parts, fmt.Sprintf("**File:** %s", fsext.PrettyPath(params.FilePath)))
	...
	case kernel.ToolNameWrite:
		var params kernel.WriteParams
		if json.Unmarshal([]byte(t.toolCall.Input), &params) == nil {
			return fmt.Sprintf("**File:** %s", fsext.PrettyPath(params.FilePath))
		}
	case kernel.ToolNameFetch:
		var params kernel.FetchParams
		if json.Unmarshal([]byte(t.toolCall.Input), &params) == nil {
			var parts []string
	...
	case kernel.ToolNameAgenticFetch:
		var params kernel.AgenticFetchParams
		if json.Unmarshal([]byte(t.toolCall.Input), &params) == nil {
			var parts []string
			if params.URL != "" {
				parts = append(parts, fmt.Sprintf("**URL:** %s", params.URL))
	...
	case kernel.ToolNameWebFetch:
		var params kernel.WebFetchParams
		if json.Unmarshal([]byte(t.toolCall.Input), &params) == nil {
			return fmt.Sprintf("**URL:** %s", params.URL)
		}
	case kernel.ToolNameGrep:
		var params kernel.GrepParams
		if json.Unmarshal([]byte(t.toolCall.Input), &params) == nil {
			var parts []string
	...
	case kernel.ToolNameDownload:
		var params kernel.DownloadParams
		if json.Unmarshal([]byte(t.toolCall.Input), &params) == nil {
			var parts []string
			parts = append(parts, fmt.Sprintf("**URL:** %s", params.URL))
	...
	case kernel.ToolNameSourcegraph:
		var params kernel.SourcegraphParams
		if json.Unmarshal([]byte(t.toolCall.Input), &params) == nil {
			var parts []string
...
func (t *baseToolMessageItem) formatResultForCopy() string {
	...
	case kernel.ToolNameDownload, kernel.ToolNameGrep, kernel.ToolNameGlob, kernel.ToolNameLS, kernel.ToolNameSourcegraph, kernel.ToolNameDiagnostics, kernel.ToolNameTodo:
		return fmt.Sprintf("```\n%s\n```", t.result.Content)
	default:
...
func (t *baseToolMessageItem) formatBashResultForCopy() string {
	...
	var meta kernel.BashResponseMetadata
	if t.result.Metadata != "" {
		json.Unmarshal([]byte(t.result.Metadata), &meta)
	}

	output := meta.Output
	if output == "" && t.result.Content != kernel.BashNoOutput {
...
func (t *baseToolMessageItem) formatViewResultForCopy() string {
	...
	var meta kernel.ViewResponseMetadata
	if t.result.Metadata != "" {
		json.Unmarshal([]byte(t.result.Metadata), &meta)
	}
...
func (t *baseToolMessageItem) formatEditResultForCopy() string {
	...
	var meta kernel.EditResponseMetadata
...
func (t *baseToolMessageItem) formatMultiEditResultForCopy() string {
	...
	var meta kernel.MultiEditResponseMetadata
	if json.Unmarshal([]byte(t.result.Metadata), &meta) != nil {
		return t.result.Content
	}
