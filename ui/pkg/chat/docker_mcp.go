...
import (
	...
	mcph "github.com/qinqd2006/crush/infra/pkg/mcp"
...
func (d *DockerMCPToolRenderContext) RenderTool(sty *styles.Styles, width int, opts *ToolRenderOpts) string {
	...
	tool := strings.TrimPrefix(opts.ToolCall.Name, "mcp_"+mcph.DockerMCPName+"_")
	...
	if len(opts.Result.Data) > 0 && strings.HasPrefix(opts.Result.MIMEType, "image/") {
		parts = append(parts, "", toolOutputImageContent(sty, string(opts.Result.Data), opts.Result.MIMEType))
	}
...
func IsDockerMCPTool(name string) bool {
	return strings.HasPrefix(name, "mcp_"+mcph.DockerMCPName+"_")
}
