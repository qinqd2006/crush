...
func (msg loadSessionMsg) lspFilePaths() []string {
	...
	for _, f := range msg.files {
		// Get the latest path from the most recent version
		p := f.LatestVersionPath
		if _, ok := seen[p]; ok {
			continue
		}
		seen[p] = struct{}{}
...
type SessionFile struct {
	FirstVersionPath     string
	LatestVersionPath    string
	FirstVersionTime     int64
	LatestVersionTime    int64
	FirstVersionHash     string
	LatestVersionHash    string
	FirstVersionContent  string
...
func (m *UI) loadSession(sessionID string) tea.Cmd {
	return func() tea.Msg {
		sess, err := m.com.Kernel().GetSession(context.Background(), sessionID)
		if err != nil {
...
func (m *UI) loadSessionFiles(sessionID string) ([]SessionFile, error) {
	...
	sessionFiles := make([]SessionFile, 0, len(files))
	for _, f := range files {
		if len(f.Versions) == 0 {
			continue
		}
		...
		first := f.Versions[0]
		last := f.Versions[len(f.Versions)-1]
		...
		if f.Content != "" {
			_, additions, deletions = diff.GenerateDiff(f.Content, f.Content, f.Path)
		}

		sessionFiles = append(sessionFiles, SessionFile{
			FirstVersionPath:     first.Path,
			LatestVersionPath:    last.Path,
			FirstVersionTime:     first.Timestamp,
			LatestVersionTime:    last.Timestamp,
			FirstVersionHash:     first.Hash,
			LatestVersionHash:    last.Hash,
			FirstVersionContent:  f.Content,
			LatestVersionContent: f.Content,
			Additions:            additions,
			Deletions:            deletions,
		})
	}
	...
	slices.SortFunc(sessionFiles, func(a, b SessionFile) int {
		if a.LatestVersionTime > b.LatestVersionTime {
			return -1
		}
		if a.LatestVersionTime < b.LatestVersionTime {
			return 1
...
func (m *UI) handleFileEvent(filePath string) tea.Cmd {
	if m.session == nil {
		return nil
	}
