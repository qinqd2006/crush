...
func (m *UI) modelInfo(width int) string {
	...
	if model != nil {
		...
		provider, ok := m.com.KernelConfig().GetProvider(model.ModelCfg.Provider)
		if ok {
			providerName = provider.Name

			// Only check reasoning if model can reason
			if model.Model.CanReason {
				if len(model.Model.ReasoningLevels) == 0 {
					...
					reasoningEffort := cmp.Or(model.ModelCfg.ReasoningEffort, model.Model.DefaultReasoningEffort)
	...
	if model != nil && m.session != nil {
		modelContext = &common.ModelContextInfo{
			...
			Cost:         m.session.Cost,
			ModelContext: model.Model.ContextWindow,
		}
	}
...
func (m *UI) drawSidebar(scr uv.Screen, area uv.Rectangle) {
	...
	for _, mcpServer := range m.com.KernelConfig().ListMCPServers() {
		if _, ok := m.mcpStates[mcpServer.Name]; ok {
			mcpsCount++
		}
	}
