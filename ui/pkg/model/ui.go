...
import (
	...
	"github.com/qinqd2006/crush/infra/pkg/home"
	"github.com/qinqd2006/crush/infra/pkg/skills"
	"github.com/qinqd2006/crush/infra/pkg/version"
	"github.com/qinqd2006/crush/kernel/pkg"
	"github.com/qinqd2006/crush/ui/pkg/anim"
	"github.com/qinqd2006/crush/ui/pkg/attachments"
	"github.com/qinqd2006/crush/ui/pkg/chat"
	"github.com/qinqd2006/crush/ui/pkg/common"
	"github.com/qinqd2006/crush/ui/pkg/completions"
	"github.com/qinqd2006/crush/ui/pkg/dialog"
	fimage "github.com/qinqd2006/crush/ui/pkg/image"
	"github.com/qinqd2006/crush/ui/pkg/logo"
	"github.com/qinqd2006/crush/ui/pkg/notification"
	"github.com/qinqd2006/crush/ui/pkg/styles"
	"github.com/qinqd2006/crush/ui/pkg/util"
)

// Conversion helpers for bridging engine and kernel types.

// customCommandsToKernel converts engine custom commands to kernel custom commands.
func customCommandsToKernel(cmds []commands.CustomCommand) []kernel.CustomCommand {
	result := make([]kernel.CustomCommand, len(cmds))
	for i, cmd := range cmds {
		args := make([]kernel.Argument, len(cmd.Arguments))
		for j, arg := range cmd.Arguments {
			args[j] = kernel.Argument{
				ID:          arg.ID,
				Title:       arg.Title,
...
// mcpPromptsToKernel converts engine MCP prompts to kernel MCP prompts.
func mcpPromptsToKernel(prompts []commands.MCPPrompt) []kernel.MCPPrompt {
	result := make([]kernel.MCPPrompt, len(prompts))
	for i, prompt := range prompts {
		args := make([]kernel.Argument, len(prompt.Arguments))
...
// errPermissionDenied is the error returned when a permission is denied.
var errPermissionDenied = errors.New("user denied permission")

// modelConfigToKernelSelectedModel converts a minimal model config to kernel selected model.
func modelConfigToKernelSelectedModel(model kernel.ModelConfig) kernel.SelectedModel {
...
type UI struct {
	...
	// lsp
	lspStates map[string]kernel.LSPClientInfo

	// mcp
	mcpStates map[string]kernel.MCPClientInfo
	...
	customCommands []kernel.CustomCommand
	mcpPrompts     []kernel.MCPPrompt
...
func New(com *common.Common, initialSessionID string, continueLast bool) *UI {
	...
	ui := &UI{
		...
		lspStates:           make(map[string]kernel.LSPClientInfo),
		mcpStates:           make(map[string]kernel.MCPClientInfo),
	...
	tuiOpts := com.KernelConfig().TUIOptions()
	ui.forceCompactMode = tuiOpts.CompactMode
	...
	if !com.KernelConfig().IsConfigured() {
	...
	globalOpts := com.KernelConfig().GetGlobalOptions()

	// disable indeterminate progress bar
	ui.progressBarEnabled = globalOpts.Progress
	// enable transparent mode
	ui.isTransparent = tuiOpts.Transparent
...
func (m *UI) shouldSendNotification() bool {
	if m.com.KernelConfig().GetGlobalOptions().DisableNotifications {
		return false
	}
...
func (m *UI) loadCustomCommands() tea.Cmd {
	return func() tea.Msg {
		customCommands, err := commands.LoadCustomCommands(".crush")
		if err != nil {
			slog.Error("Failed to load custom commands", "error", err)
		}
		return userCommandsLoadedMsg{Commands: customCommandsToKernel(customCommands)}
	}
}
...
func (m *UI) loadMCPrompts() tea.Msg {
	...
	return mcpPromptsLoadedMsg{Prompts: mcpPromptsToKernel(prompts)}
}
...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case loadSessionMsg:
		...
		msgs, err := m.com.KernelWorkspace.ListMessages(context.Background(), m.session.ID)
		if err != nil {
	...
	case sessionFilesUpdatesMsg:
		...
		for _, f := range msg.sessionFiles {
			paths = append(paths, f.LatestVersionPath)
		}
	...
	// Kernel event handlers - these receive decoupled kernel events
	case common.KernelEventMsg[kernel.Session]:
		if cmd := m.handleKernelSessionEvent(msg.Event); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case common.KernelEventMsg[kernel.Message]:
		if cmd := m.handleKernelMessageEvent(msg.Event); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case common.KernelEventMsg[kernel.Notification]:
		if cmd := m.handleKernelNotificationEvent(msg.Event); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case common.KernelEventMsg[kernel.PermissionRequest]:
		if cmd := m.handleKernelPermissionRequestEvent(msg.Event); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case common.KernelEventMsg[kernel.PermissionNotification]:
		if cmd := m.handleKernelPermissionNotificationEvent(msg.Event); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case common.KernelEventMsg[kernel.MCPEvent]:
		if cmd := m.handleKernelMCPEvent(msg.Event); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case common.KernelEventMsg[kernel.LSPEvent]:
		m.handleKernelLSPEvent(msg.Event)
	case common.KernelEventMsg[kernel.HistoryFile]:
		if cmd := m.handleKernelHistoryEvent(msg.Event); cmd != nil {
			cmds = append(cmds, cmd)
		}
	...
	// at this point this can only handle [kernel.Attachment] message, and we
...
func (m *UI) setSessionMessages(kernelMsgs []kernel.Message) tea.Cmd {
	var cmds []tea.Cmd

	// Convert to pointers for compatibility with helper functions
	kernelMsgPtrs := make([]*kernel.Message, len(kernelMsgs))
	for i := range kernelMsgs {
		kernelMsgPtrs[i] = &kernelMsgs[i]
	}

	// Build tool result map to link tool calls with their results
	toolResultMap := chat.BuildToolResultMap(kernelMsgPtrs)
	if len(kernelMsgs) > 0 {
		m.lastUserMessageTime = kernelMsgs[0].CreatedAt
	}

	// Add messages to chat with linked tool results
	items := make([]chat.MessageItem, 0, len(kernelMsgs)*2)
	for i, kernelMsg := range kernelMsgs {
		switch kernelMsg.Role {
		case kernel.RoleUser:
			m.lastUserMessageTime = kernelMsg.CreatedAt
			items = append(items, chat.ExtractMessageItems(m.com.Styles, kernelMsgPtrs[i], toolResultMap)...)
		case kernel.RoleAssistant:
			items = append(items, chat.ExtractMessageItems(m.com.Styles, kernelMsgPtrs[i], toolResultMap)...)
			if kernelMsg.FinishPart() != nil && kernelMsg.FinishReason() == kernel.FinishReasonEndTurn {
				cfgProvider := m.com.KernelConfig()
				infoItem := chat.NewAssistantInfoItem(m.com.Styles, kernelMsgPtrs[i], cfgProvider, time.Unix(m.lastUserMessageTime, 0))
...
func (m *UI) loadNestedToolCalls(items []chat.MessageItem) {
	for _, item := range items {
		...
		// Fetch nested messages using kernel workspace (returns kernel.Message directly).
		nestedMsgs, err := m.com.KernelWorkspace.ListMessages(context.Background(), agentSessionID)
		if err != nil || len(nestedMsgs) == 0 {
			continue
		}

		// Build tool result map for nested messages.
		nestedKernelMsgPtrs := make([]*kernel.Message, len(nestedMsgs))
		for i := range nestedMsgs {
			nestedKernelMsgPtrs[i] = &nestedMsgs[i]
		}
		nestedToolResultMap := chat.BuildToolResultMap(nestedKernelMsgPtrs)

		// Extract nested tool items.
		var nestedTools []chat.ToolMessageItem
		for i := range nestedMsgs {
			nestedItems := chat.ExtractMessageItems(m.com.Styles, nestedKernelMsgPtrs[i], nestedToolResultMap)
...
func (m *UI) handleDialogMsg(msg tea.Msg) tea.Cmd {
	...
	case dialog.ActionToggleNotifications:
		currentDisabled := m.com.KernelConfig().GetGlobalOptions().DisableNotifications
		disabled := !currentDisabled
		if err := m.com.Workspace.SetConfigField(kernel.ScopeGlobal, "options.disable_notifications", disabled); err != nil {
			cmds = append(cmds, util.ReportError(err))
		} else {
			status := "enabled"
			if disabled {
	...
	case dialog.ActionToggleThinking:
		cmds = append(cmds, func() tea.Msg {
			coder := m.com.KernelConfig().GetCoderAgent()
			modelType := coder.CurrentModel.Type
			selectedModel, ok := m.com.KernelConfig().GetSelectedModel(modelType)
			if !ok {
				return util.ReportError(errors.New("model configuration not found"))()
			}

			selectedModel.Think = !selectedModel.Think
			if err := m.com.Workspace.UpdatePreferredModel(kernel.ScopeGlobal, modelType, selectedModel); err != nil {
				return util.ReportError(err)()
			}
	...
	case dialog.ActionToggleTransparentBackground:
		cmds = append(cmds, func() tea.Msg {
			isTransparent := m.com.KernelConfig().TUIOptions().Transparent
			newValue := !isTransparent
			if err := m.com.Workspace.SetConfigField(kernel.ScopeGlobal, "options.tui.transparent", newValue); err != nil {
				return util.ReportError(err)()
			}
	...
	case dialog.ActionSelectModel:
		...
		var (
			...
			isConfigured = func() bool { _, ok := m.com.KernelConfig().GetProvider(providerID); return ok }
		)
		...
		if err := m.com.Workspace.UpdatePreferredModel(kernel.ScopeGlobal, msg.ModelType, msg.Model); err != nil {
			cmds = append(cmds, util.ReportError(err))
		} else if _, ok := m.com.KernelConfig().GetSmallModel(); !ok {
			// Ensure small model is set is unset.
			smallModel := m.com.Workspace.GetDefaultSmallModel(providerID)
			if err := m.com.Workspace.UpdatePreferredModel(kernel.ScopeGlobal, kernel.SelectedModelTypeSmall, modelConfigToKernelSelectedModel(smallModel)); err != nil {
				cmds = append(cmds, util.ReportError(err))
			}
		}
	...
	case dialog.ActionSelectReasoningEffort:
		...
		selectedModel, ok := m.com.KernelConfig().GetSelectedModel(modelType)
		if !ok {
			cmds = append(cmds, util.ReportError(errors.New("model configuration not found")))
			break
		}

		selectedModel.ReasoningEffort = msg.Effort
		if err := m.com.Workspace.UpdatePreferredModel(kernel.ScopeGlobal, modelType, selectedModel); err != nil {
			cmds = append(cmds, util.ReportError(err))
			break
		}
	...
	case dialog.ActionPermissionResponse:
		...
		permReq := kernel.PermissionRequest{
			ID:          msg.Permission.ID,
			SessionID:   msg.Permission.SessionID,
			ToolCallID:  msg.Permission.ToolCallID,
			ToolName:    msg.Permission.ToolName,
			Description: msg.Permission.Description,
			Action:      msg.Permission.Action,
			Params:      msg.Permission.Params,
			Path:        msg.Permission.Path,
		}
		switch msg.Action {
		case dialog.PermissionAllow:
			m.com.Workspace.PermissionGrant(permReq)
		case dialog.PermissionAllowForSession:
			m.com.Workspace.PermissionGrantPersistent(permReq)
		case dialog.PermissionDeny:
			m.com.Workspace.PermissionDeny(permReq)
		}
...
func (m *UI) openAuthenticationDialog(provider kernel.Provider, model kernel.SelectedModel, modelType kernel.SelectedModelType) tea.Cmd {
	var (
		dlg dialog.Dialog
		cmd tea.Cmd
	...
	case uiChat, uiLanding:
		...
		case uiFocusEditor:
			...
			case key.Matches(msg, m.keyMap.Editor.SendMessage):
				...
				m.attachments.Reset()
				if len(value) == 0 && !kernel.ContainsTextAttachment(attachments) {
					return nil
				}
			...
			default:
				...
				if msg.String() == "@" && !m.completionsOpen {
					...
					if curIdx == 0 || (curIdx > 0 && isWhitespace(curValue[curIdx-1])) {
						...
						completions := m.com.KernelConfig().TUIOptions().Completions
						cmds = append(cmds, m.completions.Open(completions.MaxDepth, completions.MaxItems))
					}
				}
...
func (m *UI) currentModelSupportsImages() bool {
	coder := m.com.KernelConfig().GetCoderAgent()
	return coder.CurrentModel.SupportsImage
}
...
func (m *UI) toggleCompactMode() tea.Cmd {
	...
	err := m.com.Workspace.SetCompactMode(kernel.ScopeGlobal, m.forceCompactMode)
	if err != nil {
		return util.ReportError(err)
	}
...
func (m *UI) insertFileCompletion(path string) tea.Cmd {
	...
	fileCmd := func() tea.Msg {
		...
		if m.hasSession() {
			...
			if lastRead != 0 {
				if info, err := os.Stat(path); err == nil && !info.ModTime().After(time.Unix(lastRead, 0)) {
					return nil
				}
			}
...
func (m *UI) sendMessage(content string, attachments ...kernel.Attachment) tea.Cmd {
	if !m.com.Kernel().IsBusy() {
		return util.ReportError(fmt.Errorf("coder agent is not initialized"))
	}

	var cmds []tea.Cmd
	if !m.hasSession() {
		newSession, err := m.com.Kernel().CreateSession(context.Background(), "New Session")
		if err != nil {
			return util.ReportError(err)
		}
		if m.forceCompactMode {
	...
	cmds = append(cmds, func() tea.Msg {
		err := m.com.Kernel().RunPrompt(context.Background(), sessionID, content, attachments...)
		if err != nil {
			isCancelErr := errors.Is(err, context.Canceled)
			isPermissionErr := errors.Is(err, errPermissionDenied)
...
func (m *UI) openPermissionsDialog(perm kernel.PermissionRequest) tea.Cmd {
	// Close any existing permissions dialog first.
	m.dialog.CloseDialog(dialog.PermissionsID)

	// Get diff mode from config.
	var opts []dialog.PermissionsOption
	if diffMode := m.com.KernelConfig().TUIOptions().DiffMode; diffMode != "" {
...
func (m *UI) handlePermissionNotification(notification kernel.PermissionNotification) {
...
func (m *UI) handleAgentNotification(n kernel.Notification) tea.Cmd {
	switch n.Type {
	case kernel.NotificationAgentFinished:
		return m.sendNotification(notification.Notification{
			Title:   "Crush is waiting...",
			Message: fmt.Sprintf("Agent's turn completed in \"%s\"", n.SessionTitle),
		})
	case kernel.NotificationReAuthenticate:
...
func (m *UI) handleReAuthenticate(providerID string) tea.Cmd {
	providers, err := m.com.KernelConfig().Providers()
	if err != nil {
		return nil
	}
	...
	return m.openAuthenticationDialog(provider, selectedModel, modelType)
}
...
func (m *UI) handleStateChanged() tea.Cmd {
	return func() tea.Msg {
		...
		return mcpStateChangedMsg{
			states: m.com.KernelWorkspace.GetMCPStates(),
		}
	}
}

func handleMCPPromptsEvent(ws kernel.Workspace, name string) tea.Cmd {
	return func() tea.Msg {
		ws.RefreshMCPPrompts(context.Background(), name)
		return nil
	}
}

func handleMCPToolsEvent(ws kernel.Workspace, name string) tea.Cmd {
	return func() tea.Msg {
...
func handleMCPResourcesEvent(ws kernel.Workspace, name string) tea.Cmd {
	return func() tea.Msg {
		ws.RefreshMCPResources(context.Background(), name)
		return nil
	}
}
...
// These handlers process decoupled kernel events and convert them
// to internal types for backward compatibility.
// ============================================================================

func (m *UI) handleKernelSessionEvent(event kernel.Event[kernel.Session]) tea.Cmd {
	sess := event.Payload

	switch event.Type {
	case kernel.EventDeleted:
	...
	default:
		if m.session != nil && m.session.ID == sess.ID {
			...
			m.session = &sess
			if !prevHasInProgress && hasInProgressTodo(m.session.Todos) && m.isAgentBusy() {
				m.todoIsSpinning = true
				m.updateLayoutAndSize()
				return m.todoSpinner.Tick
			}
			m.updateLayoutAndSize()
		}
	}
...
func (m *UI) handleKernelMessageEvent(event kernel.Event[kernel.Message]) tea.Cmd {
	kernelMsg := &event.Payload

	// Check if this is a child session message for an agent tool.
	if m.session == nil {
		return nil
	}
	if kernelMsg.SessionID != m.session.ID {
		// This might be a child session message from an agent tool.
		return m.handleChildSessionMessageFromKernel(kernelMsg)
	}

	switch event.Type {
	case kernel.EventCreated:
		return m.appendSessionMessageFromKernel(kernelMsg)
	case kernel.EventUpdated:
		return m.updateSessionMessageFromKernel(kernelMsg)
	case kernel.EventDeleted:
		m.chat.RemoveMessage(kernelMsg.ID)
	}
	return nil
}

func (m *UI) handleKernelNotificationEvent(event kernel.Event[kernel.Notification]) tea.Cmd {
	return m.handleAgentNotification(event.Payload)
}

func (m *UI) handleKernelPermissionRequestEvent(event kernel.Event[kernel.PermissionRequest]) tea.Cmd {
	req := event.Payload

	var cmds []tea.Cmd
	if cmd := m.openPermissionsDialog(req); cmd != nil {
		cmds = append(cmds, cmd)
	}
	if cmd := m.sendNotification(notification.Notification{
		Title:   "Crush is waiting...",
		Message: fmt.Sprintf("Permission required to execute \"%s\"", req.ToolName),
	}); cmd != nil {
		cmds = append(cmds, cmd)
	}
	return tea.Batch(cmds...)
...
func (m *UI) handleKernelPermissionNotificationEvent(event kernel.Event[kernel.PermissionNotification]) tea.Cmd {
	m.handlePermissionNotification(event.Payload)
	return nil
}

func (m *UI) handleKernelMCPEvent(event kernel.Event[kernel.MCPEvent]) tea.Cmd {
	switch event.Payload.Type {
	case kernel.MCPEventStateChanged:
		return m.handleStateChangedFromKernel()
	case kernel.MCPEventPromptsListChanged:
		return handleMCPPromptsEvent(m.com.Workspace, event.Payload.Name)
	case kernel.MCPEventToolsListChanged:
		return handleMCPToolsEvent(m.com.Workspace, event.Payload.Name)
	case kernel.MCPEventResourcesListChanged:
		return handleMCPResourcesEvent(m.com.Workspace, event.Payload.Name)
	}
	return nil
}

func (m *UI) handleKernelLSPEvent(event kernel.Event[kernel.LSPEvent]) {
	m.lspStates = m.com.KernelWorkspace.GetLSPStates()
}

func (m *UI) handleKernelHistoryEvent(event kernel.Event[kernel.HistoryFile]) tea.Cmd {
	// The history event is for the current session
	// We just need to reload the session files
	return m.handleFileEvent(event.Payload.Path)
}

// openPermissionsDialogFromKernel opens permissions dialog with kernel permission request
// Note: This is now inlined into handleKernelPermissionRequestEvent

// handleStateChangedFromKernel handles MCP state changed event from kernel
func (m *UI) handleStateChangedFromKernel() tea.Cmd {
	m.com.Workspace.UpdateAgentModel(context.Background())
	m.mcpStates = m.com.KernelWorkspace.GetMCPStates()
	return tea.Batch(
...
// appendSessionMessageFromKernel appends a session message from kernel message
func (m *UI) appendSessionMessageFromKernel(kernelMsg *kernel.Message) tea.Cmd {
	var cmds []tea.Cmd

	existing := m.chat.MessageItem(kernelMsg.ID)
	if existing != nil {
		// message already exists, skip
		return nil
	}

	var cfgProvider kernel.ConfigProvider
	if m.com.KernelWorkspace != nil {
		cfgProvider = m.com.KernelWorkspace.Config()
	} else {
		cfgProvider = m.com.KernelConfig()
	}
	...
	case kernel.RoleTool:
		for _, tr := range kernelMsg.ToolResults() {
...
// updateSessionMessageFromKernel updates a session message from kernel message
func (m *UI) updateSessionMessageFromKernel(kernelMsg *kernel.Message) tea.Cmd {
	var cmds []tea.Cmd
	existingItem := m.chat.MessageItem(kernelMsg.ID)
	var cfgProvider kernel.ConfigProvider
	if m.com.KernelWorkspace != nil {
		cfgProvider = m.com.KernelWorkspace.Config()
	} else {
	...
	if !shouldRenderAssistant && len(kernelMsg.ToolCalls()) > 0 && existingItem != nil {
		m.chat.RemoveMessage(kernelMsg.ID)
		if infoItem := m.chat.MessageItem(chat.AssistantInfoID(kernelMsg.ID)); infoItem != nil {
			m.chat.RemoveMessage(chat.AssistantInfoID(kernelMsg.ID))
		}
	}
	...
	for _, tc := range kernelMsg.ToolCalls() {
		...
		if existingToolItem == nil {
			items = append(items, chat.NewToolMessageItem(m.com.Styles, kernelMsg.ID, tc, nil, false))
		}
	}
...
// handleChildSessionMessageFromKernel handles messages from child sessions (agent tools) via kernel events
func (m *UI) handleChildSessionMessageFromKernel(kernelMsg *kernel.Message) tea.Cmd {
	var cmds []tea.Cmd

	// Only process messages with tool calls or results.
	if len(kernelMsg.ToolCalls()) == 0 && len(kernelMsg.ToolResults()) == 0 {
		return nil
	}

	// Check if this is an agent tool session and parse it.
	childSessionID := kernelMsg.SessionID
	...
	for _, tc := range kernelMsg.ToolCalls() {
		...
		if !found {
			nestedItem := chat.NewToolMessageItem(m.com.Styles, kernelMsg.ID, tc, nil, false)
