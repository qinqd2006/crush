...
func hasIncompleteTodos(todos []kernel.Todo) bool {
...
func hasInProgressTodo(todos []kernel.Todo) bool {
	for _, todo := range todos {
...
func todoPill(todos []kernel.Todo, spinnerView string, focused, panelFocused bool, t *styles.Styles) string {
	if !hasIncompleteTodos(todos) {
		return ""
	}

	completed := 0
	var currentTodo *kernel.Todo
	for i := range todos {
...
func todoList(todos []kernel.Todo, spinnerView string, t *styles.Styles, width int) string {
	return chat.FormatTodosList(t, todos, spinnerView, width)
}
