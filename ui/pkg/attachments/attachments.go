...
type Attachments struct {
	renderer *Renderer
	keyMap   Keymap
	list     []kernel.Attachment
	deleting bool
}

func (m *Attachments) List() []kernel.Attachment { return m.list }
func (m *Attachments) Reset()                    { m.list = nil }

func (m *Attachments) Update(msg tea.Msg) bool {
	switch msg := msg.(type) {
	case kernel.Attachment:
...
func (r *Renderer) Render(attachments []kernel.Attachment, deleting bool, width int) string {
	var chips []string
...
func (r *Renderer) icon(a kernel.Attachment) lipgloss.Style {
	if a.IsImage() {
		return r.imageStyle
	}
	return r.textStyle
