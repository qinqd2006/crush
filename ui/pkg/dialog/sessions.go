...
func NewSessions(com *common.Common, selectedSessionID string) (*Session, error) {
	...
	s.com = com
	sessions, err := com.KernelWorkspace.ListSessions(context.TODO())
	if err != nil {
		return nil, err
	}
...
func (s *Session) removeSession(id string) {
	var newSessions []kernel.Session
...
func (s *Session) deleteSessionCmd(id string) tea.Cmd {
	return func() tea.Msg {
		err := s.com.KernelWorkspace.DeleteSession(context.TODO(), id)
		if err != nil {
...
func (s *Session) updateSession(session kernel.Session) {
...
func (s *Session) updateSessionCmd(session kernel.Session) tea.Cmd {
	return func() tea.Msg {
		_, err := s.com.KernelWorkspace.SaveSession(context.TODO(), session)
		if err != nil {
			return util.NewErrorMsg(err)
		}
		return nil
