...
import (
	...
	"github.com/qinqd2006/crush/ui/pkg/list"
...
func NewCommands(com *common.Common, sessionID string, hasSession, hasTodos, hasQueue bool, customCommands []kernel.CustomCommand, mcpPrompts []kernel.MCPPrompt) (*Commands, error) {
	c := &Commands{
		com:            com,
	...
	if available, known := com.KernelWorkspace.GetDockerMCPAvailability(); known {
...
func checkDockerMCPAvailabilityCmd(kw kernel.Workspace) tea.Cmd {
	return func() tea.Msg {
		return dockerMCPAvailabilityCheckedMsg{available: kw.RefreshDockerMCPAvailability()}
	}
}
...
func (c *Commands) InitialCmd() tea.Cmd {
	...
	return checkDockerMCPAvailabilityCmd(c.com.KernelWorkspace)
}
...
func (c *Commands) defaultCommands() []*CommandItem {
	...
	cfg := c.com.Config()
	if agentCfg, ok := cfg.Agents[string(kernel.AgentCoder)]; ok {
	...
	if c.hasSession {
		...
		agentCfg := cfgPrime.Agents[string(kernel.AgentCoder)]
...
func (c *Commands) SetCustomCommands(customCommands []kernel.CustomCommand) {
...
func (c *Commands) SetMCPPrompts(mcpPrompts []kernel.MCPPrompt) {
