...
type Permissions struct {
	...
	permission     kernel.PermissionRequest
...
func NewPermissions(com *common.Common, perm kernel.PermissionRequest, opts ...PermissionsOption) *Permissions {
	h := help.New()
...
func (p *Permissions) hasDiffView() bool {
	...
	case kernel.ToolNameEdit, kernel.ToolNameWrite, kernel.ToolNameMultiEdit:
		return true
	}
	return false
}
...
func (p *Permissions) renderHeader(contentWidth int) string {
	...
	pathLine := p.renderKeyValue("Path", util.PrettyPath(p.permission.Path), contentWidth)
	...
	case kernel.ToolNameBash:
		if params, ok := p.permission.Params.(kernel.BashPermissionsParams); ok {
			lines = append(lines, p.renderKeyValue("Desc", params.Description, contentWidth))
		}
	case kernel.ToolNameDownload:
		if params, ok := p.permission.Params.(kernel.DownloadPermissionsParams); ok {
			lines = append(lines, p.renderKeyValue("URL", params.URL, contentWidth))
			lines = append(lines, p.renderKeyValue("File", util.PrettyPath(params.FilePath), contentWidth))
		}
	case kernel.ToolNameEdit, kernel.ToolNameWrite, kernel.ToolNameMultiEdit, kernel.ToolNameView:
		var filePath string
		switch params := p.permission.Params.(type) {
		case kernel.EditPermissionsParams:
			filePath = params.FilePath
		case kernel.WritePermissionsParams:
			filePath = params.FilePath
		case kernel.MultiEditPermissionsParams:
			filePath = params.FilePath
		case kernel.ViewPermissionsParams:
			filePath = params.FilePath
		}
		if filePath != "" {
			lines = append(lines, p.renderKeyValue("File", util.PrettyPath(filePath), contentWidth))
		}
	case kernel.ToolNameLS:
		if params, ok := p.permission.Params.(kernel.LSPermissionsParams); ok {
			lines = append(lines, p.renderKeyValue("Directory", util.PrettyPath(params.Path), contentWidth))
		}
	}
...
func (p *Permissions) renderBashContent(width int) string {
	params, ok := p.permission.Params.(kernel.BashPermissionsParams)
	if !ok {
		return ""
	}
...
func (p *Permissions) renderEditContent(contentWidth int) string {
	params, ok := p.permission.Params.(kernel.EditPermissionsParams)
	if !ok {
		return ""
	}
...
func (p *Permissions) renderWriteContent(contentWidth int) string {
	params, ok := p.permission.Params.(kernel.WritePermissionsParams)
	if !ok {
		return ""
	}
	return p.renderDiff(params.FilePath, params.OldContent, params.NewContent, contentWidth)
...
func (p *Permissions) renderMultiEditContent(contentWidth int) string {
	params, ok := p.permission.Params.(kernel.MultiEditPermissionsParams)
	if !ok {
		return ""
	}
	return p.renderDiff(params.FilePath, params.OldContent, params.NewContent, contentWidth)
...
func (p *Permissions) renderDiff(filePath, oldContent, newContent string, contentWidth int) string {
	...
	formatter := common.DiffFormatter(p.com.Styles).
		Before(util.PrettyPath(filePath), oldContent).
		After(util.PrettyPath(filePath), newContent).
...
func (p *Permissions) renderDownloadContent(width int) string {
	params, ok := p.permission.Params.(kernel.DownloadPermissionsParams)
	if !ok {
		return ""
	}

	content := fmt.Sprintf("URL: %s\nFile: %s", params.URL, util.PrettyPath(params.FilePath))
	if params.Timeout > 0 {
...
func (p *Permissions) renderFetchContent(width int) string {
	params, ok := p.permission.Params.(kernel.FetchPermissionsParams)
	if !ok {
		return ""
	}
...
func (p *Permissions) renderAgenticFetchContent(width int) string {
	params, ok := p.permission.Params.(kernel.AgenticFetchPermissionsParams)
	if !ok {
		return ""
	}
...
func (p *Permissions) renderViewContent(width int) string {
	params, ok := p.permission.Params.(kernel.ViewPermissionsParams)
	if !ok {
		return ""
	}

	content := fmt.Sprintf("File: %s", util.PrettyPath(params.FilePath))
	if params.Offset > 0 {
...
func (p *Permissions) renderLSContent(width int) string {
	params, ok := p.permission.Params.(kernel.LSPermissionsParams)
	if !ok {
		return ""
	}

	content := fmt.Sprintf("Directory: %s", util.PrettyPath(params.Path))
