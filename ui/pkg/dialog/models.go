...
// Kernel returns the corresponding kernel model type.
func (mt ModelType) Kernel() kernel.SelectedModelType {
	switch mt {
	case ModelTypeLarge:
		return kernel.SelectedModelTypeLarge
	case ModelTypeSmall:
		return kernel.SelectedModelTypeSmall
	default:
		return ""
	}
}
...
func NewModels(com *common.Common, isOnboarding bool) (*Models, error) {
	...
	var err error
	m.providers, err = com.KernelConfig().Providers()
	if err != nil {
...
func (m *Models) isSelectedConfigured() bool {
	...
	providerID := modelItem.prov.ID
	kcfg := m.com.KernelConfig().Get()
	_, isConfigured := kcfg.Providers[providerID]
	return isConfigured
}
...
func (m *Models) setProviderItems() error {
	...
	selectedType := m.modelType.Kernel()
	currentModel, _ := kcfg.Models[selectedType]
	recentItems := kcfg.RecentModels[selectedType]

	// Track providers already added to avoid duplicates
	addedProviders := make(map[string]bool)

	// Get a list of known providers to compare against
	knownProviders, err := m.com.KernelConfig().Providers()
	if err != nil {
		return fmt.Errorf("failed to get providers: %w", err)
	}

	containsProviderFunc := func(id string) func(p kernel.Provider) bool {
		return func(p kernel.Provider) bool {
			return p.ID == id
		}
	}
	...
	for id, p := range kcfg.Providers {
		...
		if !slices.ContainsFunc(knownProviders, containsProviderFunc(id)) ||
			...
			for _, model := range p.Models {
				item := NewModelItem(t, p, model, m.modelType, false)
				group.AppendItems(item)
				itemsMap[item.ID()] = item
				if model.ID == currentModel.Model && p.ID == currentModel.Provider {
	...
	slices.SortStableFunc(m.providers, func(a, b kernel.Provider) int {
		switch {
		case a.ID == "hyper":
			return -1
		case b.ID == "hyper":
	...
	for _, provider := range m.providers {
		...
		providerConfig, providerConfigured := kcfg.Providers[providerID]
		// Skip disabled providers
		if providerConfigured {
			// Note: kernel Provider doesn't have Disable field yet
			// This is handled by the existing providers check above
		}
	...
	if len(recentItems) > 0 {
		...
		var validRecentItems []kernel.SelectedModel
		...
		if len(validRecentItems) != len(recentItems) {
			...
			if err := m.com.KernelConfig().SetConfigField(kernel.ScopeGlobal, fmt.Sprintf("recent_models.%s", selectedType), validRecentItems); err != nil {
